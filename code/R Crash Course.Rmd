...
The `group_by()` command is also useful in a number of other conditions. Suppose we are interested in finding the respondents of each gender who give him a higher than average score than other people of the same gender. In this case, we can group respondents and then `filter()` respondents who rate higher than the average in *each* group.
...
Here are some key functions and concepts that especially come in handy for modeling tasks. Notably, we start with the ability to "map" or iteratively pass functions to values stored in vectors. We will use the `purrr` package for this (which lets us bypass the use of `for` loops).
